...
async def test_tags_migration_preserves_existing_pages_and_downgrades() -> None:
    config = Config("alembic.ini")
    await asyncio.to_thread(command.downgrade, config, "20260823_0005")
    ...
    try:
        ...
        tags = await connection.fetchval("SELECT tags FROM saved_pages WHERE id = $1", page_id)
        nullable = await connection.fetchval(
            """
            SELECT is_nullable
            FROM information_schema.columns
            WHERE table_name = 'saved_pages' AND column_name = 'tags'
            """
        )
