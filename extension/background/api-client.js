...
/* Pairing links a second client to the SAME anonymous user, so both see one saved-page
   ...
   This extension only MINTS codes - it holds the authenticated session, so it is the only
   client that can. Redeeming happens on the dashboard (dashboard/src/api/session.ts),
   which is also where the code normalizing lives, since that is where a human types one. */
